...
function distToSegment(lng, lat, coords) {
  let m = Infinity;
  for (const [cl, ct] of coords) {
    const d = Math.sqrt((lng - cl) ** 2 + (lat - ct) ** 2);
    if (d < m) m = d;
  }
  return m;
}

export function findNearestSegment(lng, lat) {
  let best = null;
  let bd = Infinity;
...
export function getSignageBySegment(quarter) {
  const result = {};
...
const SIGNAGE_IMPACT = { walkability: 5, roadSafety: 4 };
const CATEGORY_WEIGHTS = { walkability: 0.192, lighting: 0.096, transport: 0.192, visibility: 0.096, accessibility: 0.192, roadSafety: 0.231 };

export function applySignageImpact(base, count) {
  if (count === 0) return base;
  const cats = ['walkability', 'lighting', 'transport', 'visibility', 'accessibility', 'roadSafety'];
  const ns = { ...base };
  cats.forEach(c => {
    const per = SIGNAGE_IMPACT[c] ?? 0;
    let b = 0;
    for (let i = 0; i < count; i++) b += per * Math.pow(0.6, i);
    ns[c] = Math.min(100, base[c] + b);
  });
